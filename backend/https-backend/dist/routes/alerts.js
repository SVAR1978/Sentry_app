...
router.post("/", authenticateJWT, async (req, res) => {
    try {
        ...
        const { title, description, severity, affectedAreas, issuedBy, expiresAt } = req.body;
        // Validation
        ...
        const alert = await prisma.safetyAlert.create({
            data: {
                ...
                severity,
                affectedAreas: (affectedAreas || []).map((a) => sanitize(a)),
...
// Deactivate an alert
// ────────────────────────────────────────────────────────────
router.patch("/:id/deactivate", authenticateJWT, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: "Unauthorized" });
