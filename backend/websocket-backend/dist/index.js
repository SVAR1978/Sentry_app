...
import { broadcastUserSessionToAdmins, } from "./services/adminRealtimeService.js";
dotenv.config();
const runEmailWorkerInThisService = (process.env.RUN_EMAIL_WORKER ?? "false").toLowerCase() === "true";
...
redisSubscriber.subscribe("user-session-events", (err, count) => {
    if (err) {
        ...
        console.log(`[Redis] Successfully subscribed to ${count} channel(s): user-session-events`);
    }
});
/**
...
redisSubscriber.on("message", (channel, message) => {
    if (channel !== "user-session-events")
        return;
    try {
        const sessionEvent = JSON.parse(message);
        console.log(`[WebSocket] Received USER_SESSION event from Redis: ${sessionEvent.action} for user ${sessionEvent.userId}`);
